...
function App() {
  ...
  const [loading, setLoading] = useState(false); // Loading state for AI Assist
  ...
  const handleAiAssist = async () => {
    ...
    try {
      const response = await axios.post("http://localhost:5000/ai-assist", {
        prompt: aiPrompt,  // Send the prompt to backend
      });

      setAiOutput(response.data.aiOutput);  // Set the AI response in state
    } catch (error) {
